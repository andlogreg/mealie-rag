...
def create_point_from_recipe(recipe: Recipe, embedding: list[float]) -> PointStruct:
    ...
    payload = {
        ...
        "category": [c.lower() for c in recipe.recipeCategory],
        "tags": [t.lower() for t in recipe.tags],
        "tools": [t.lower() for t in recipe.tools],
        "method": [m.lower() for m in recipe.method],
        "ingredient_count": len(recipe.recipeIngredients),
